...
Logging (text/JSON, runtime level control), per-minute ingest histograms, backpressure metrics, and Prometheus monitoring integration.
...
## Runtime logging control (admin UI)

Added in v5.14.0. You can change log levels on a **live** server without a
restart from **Admin → Logging** (`/admin/logging`):

- **Global level** — raise or lower the root level for the whole process.
- **Per-logger level** — target an individual managed logger (e.g. a single
  subscriber or calculator namespace) without turning up noise everywhere.

Changes take effect immediately and, in worker-pool deployments, are broadcast to
every worker process via the `SET_LOG_LEVEL` control message, so a level change
applies across the whole cluster rather than just the web process. This pairs
well with the hot-path logging design: per-message DEBUG logging is gated behind
`logger.isEnabledFor(DEBUG)`, so flipping a logger to DEBUG at runtime turns on
detailed tracing for just that component and flipping it back removes the cost.

> Runtime level changes are in-memory for the running process(es); they are not
> persisted to `application.yaml`. To make a level permanent, also set it in
> configuration.

## Observability: per-minute ingest histogram

Added in v5.13.0 (widened to a 30-minute window in v5.13.1). The **DAG Details**
page renders a true per-minute message-throughput histogram so you can see ingest
shape over the last 30 minutes at a glance — useful for spotting bursts, stalls,
or the effect of a freeze/drain. The data is also available in the DAG view model
(`dag_stats['ingest_buckets']`) for programmatic use.
