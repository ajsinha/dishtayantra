# DishtaYantra v1.7.0 - Quick Start Guide

**Release Date:** January 2025

This guide covers the new features introduced in v1.7.0, including the **CPP Manager** for high-performance C++ calculator integration.

## 🆕 What's New in v1.7.0

### C++ Calculator Integration via CPP Manager
- **cpp_config.json** - Centralized configuration for C++ modules
- **CPP Manager** - Automatic module loading and calculator management
- **Web UI** - C++ Management dashboard at `/cpp`
- **~100ns overhead** - Ultra-low latency native calculations
- **8 Pre-built calculators** - Math, Statistics, Matrix, String, Validation, Pricing, Risk
...
3. For C++ calculators: CMake, pybind11, C++17 compiler
4. For Java calculators: JDK 11+ (optional)
...
## 🆕 New in v1.7.0: CPP Manager & pybind11 Integration

### What is the CPP Manager?

The CPP Manager provides **centralized management** of C++ pybind11 modules for ultra-high-performance native calculator integration:

- **Config-Based Module Management**: Configure C++ modules via `config/cpp_config.json`
- **Automatic Module Loading**: Modules load automatically on application startup
- **Calculator Caching**: Instances are cached for optimal performance
- **~100ns Overhead**: Direct function calls with zero serialization
- **Web UI Management**: Load, unload, build modules from `/cpp`

### Build C++ Module (Required for C++ Calculators)
...
Or let the UI build it for you at `/cpp`!

### Configure C++ Settings

Edit `config/cpp_config.json`:
...
```json
{
    ...
    "modules": [
        {
            "name": "dishtayantra_cpp",
            "enabled": true,
            "description": "Core DishtaYantra C++ calculators"
        }
    ],
    "calculators": [
        {
            ...
            "default_config": {"operation": "sum"}
        }
    ]
}
```

### Using C++ Calculators in DAGs

Reference C++ calculators by name in your DAG:
...
### Available C++ Calculators

| Calculator | Description |
|------------|-------------|
| `PassthroughCalculator` | Returns input unchanged (testing) |
| `MathCalculator` | Sum, mean, variance, product, max, min |
| `StatisticsCalculator` | Percentiles, std dev, range |
| `MatrixCalculator` | Transpose, trace, determinant |
| `StringTransformCalculator` | Uppercase, lowercase, reverse, hash |
| `DataValidationCalculator` | Required fields, type checking |
| `TradePricingCalculator` | Commission, tax, fees calculation |
| `RiskMetricsCalculator` | VaR, Greeks-based risk |

### C++ Management UI

Visit `http://localhost:5002/cpp` to:
- View module status
- Load/unload/reload modules
- Build modules via CMake
- View calculator definitions
- Test calculators

---

## JVM Manager & Py4J Integration (v1.6.0)
